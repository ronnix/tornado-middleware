...
from tornado.web import authenticated, Application, \
    asynchronous, url, HTTPError, StaticFileHandler, _create_signature, \
    create_signed_value
...
from tornado_middleware import MiddlewareRequestHandler
...
class CookieTestRequestHandler(MiddlewareRequestHandler):
...
class CookieTest(AsyncHTTPTestCase, LogTrapTestCase):
    def get_app(self):
        class SetCookieHandler(MiddlewareRequestHandler):
            def get(self):
        ...
        class GetCookieHandler(MiddlewareRequestHandler):
            def get(self):
                self.write(self.get_cookie("foo", "default"))

        class SetCookieDomainHandler(MiddlewareRequestHandler):
            def get(self):
        ...
        class SetCookieSpecialCharHandler(MiddlewareRequestHandler):
            def get(self):
                self.set_cookie("equals", "a=b")
                self.set_cookie("semicolon", "a;b")
                self.set_cookie("quote", 'a"b')

        class SetCookieOverwriteHandler(MiddlewareRequestHandler):
            def get(self):
...
class AuthRedirectRequestHandler(MiddlewareRequestHandler):
...
class ConnectionCloseHandler(MiddlewareRequestHandler):
...
class EchoHandler(MiddlewareRequestHandler):
...
class TypeCheckHandler(MiddlewareRequestHandler):
    def prepare(self):
        self.errors = {}
...
class DecodeArgHandler(MiddlewareRequestHandler):
...
class LinkifyHandler(MiddlewareRequestHandler):
    def get(self):
        self.render("linkify.html", message="http://example.com")


class UIModuleResourceHandler(MiddlewareRequestHandler):
    def get(self):
        self.render("page.html", entries=[1, 2])


class OptionalPathHandler(MiddlewareRequestHandler):
    def get(self, path):
        self.write({"path": path})


class FlowControlHandler(MiddlewareRequestHandler):
...
class MultiHeaderHandler(MiddlewareRequestHandler):
    def get(self):
...
class RedirectHandler(MiddlewareRequestHandler):
    def get(self):
...
class EmptyFlushCallbackHandler(MiddlewareRequestHandler):
    @gen.engine
    @asynchronous
    def get(self):
...
class HeaderInjectionHandler(MiddlewareRequestHandler):
    def get(self):
        try:
...
class ErrorResponseTest(AsyncHTTPTestCase, LogTrapTestCase):
    def get_app(self):
        class DefaultHandler(MiddlewareRequestHandler):
            def get(self):
                if self.get_argument("status", None):
                    raise HTTPError(int(self.get_argument("status")))
                1 / 0

        class WriteErrorHandler(MiddlewareRequestHandler):
            def get(self):
                if self.get_argument("status", None):
        ...
        class GetErrorHtmlHandler(MiddlewareRequestHandler):
            def get(self):
                if self.get_argument("status", None):
                    self.send_error(int(self.get_argument("status")))
                else:
        ...
        class FailedWriteErrorHandler(MiddlewareRequestHandler):
            def get(self):
                1 / 0
...
class StaticFileTest(AsyncHTTPTestCase, LogTrapTestCase):
    def get_app(self):
        class StaticUrlHandler(MiddlewareRequestHandler):
            def get(self, path):
                self.write(self.static_url(path))

        class AbsoluteStaticUrlHandler(MiddlewareRequestHandler):
            include_host = True

            def get(self, path):
                self.write(self.static_url(path))

        class OverrideStaticUrlHandler(MiddlewareRequestHandler):
            def get(self, path):
...
class ClearHeaderTest(SimpleHandlerTestCase):
    class Handler(MiddlewareRequestHandler):
        def get(self):
